...
class DPAD():
    ...
    def _make_task(self,config,scanner):
        ...
        for i in range(config.num_file):
            file_name = config.input_path+i+'.dat'
            if is_file_normal(file_name):
                input_data = np.array(load_data(file_name))
                module_data = split_data_into_module(input_data,scanner.nb_blocks_per_ring)
                corrected_module_data = module_correction(module_data,config.nb_blocks_per_ring,config.relation_moduleid,config.relation_crystalid)
